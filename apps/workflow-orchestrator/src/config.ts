...
export function loadWorkflowOrchestratorConfig(
  ...
  const pollMs = readPollMs(env.OSVA_WORKFLOW_ORCHESTRATOR_POLL_MS);
...
function readPollMs(value: string | undefined): number {
  ...
  if (
    ...
    throw new Error(
      `OSVA_WORKFLOW_ORCHESTRATOR_POLL_MS must be an integer between ${String(MIN_POLL_MS)} and ${String(MAX_POLL_MS)}.`,
    );
  }
