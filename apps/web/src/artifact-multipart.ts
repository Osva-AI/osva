...
import { createArtifactFormFieldsSchema } from "@osva/contracts/schemas";
...
export interface ArtifactMultipartUploadHooks {
  onUploadReady(
    fields: ParsedArtifactMultipartFields,
    fileStream: Readable,
    fileMediaType?: string,
  ): void;
}

/** Multipart overhead allowance for optional Content-Length early reject. */
export const ARTIFACT_MULTIPART_OVERHEAD_BYTES = 1_048_576;

export function rejectOversizedArtifactRequestBody(
  ...
  if (contentLength > maxBytes + ARTIFACT_MULTIPART_OVERHEAD_BYTES) {
...
export async function parseArtifactMultipartUpload(
  request: IncomingMessage,
  contentType: string,
  maxBytes: number,
  hooks: ArtifactMultipartUploadHooks,
): Promise<void> {
