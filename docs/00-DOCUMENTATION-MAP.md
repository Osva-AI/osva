# OSVA Documentation Map

## Product
- `product/PRODUCT_VISION.md`
- `product/PRODUCT_SCOPE.md`
- `product/PRODUCT_PRINCIPLES.md`
- `product/PRODUCT_CAPABILITY_MAP.md`
- `product/USER_JOURNEYS.md`
- `product/GLOSSARY.md`
- `product/AI_OFFICE_VISION.md`

## Architecture
- `architecture/REFERENCE_ARCHITECTURE.md`
- `architecture/ARCHITECTURAL_INVARIANTS.md`
- `architecture/DOMAIN_MODEL.md`
- `architecture/CONTROL_EXECUTION_PLANES.md`
- `architecture/DATA_MODEL.md`
- `architecture/COMPATIBILITY_AND_MIGRATION.md`
- `architecture/DEPLOYMENT_TOPOLOGIES.md`

## Public contracts
- `contracts/README.md`
- `contracts/AGENT_MANIFEST_V1.md`
- `contracts/RUNTIME_PROTOCOL_V1.md`
- `contracts/RUN_STATE_MACHINE_V1.md`
- `contracts/WORKFLOW_DEFINITION_V1.md`
- `contracts/WORKFLOW_DEFINITION_V2.md`
- `contracts/EVENT_ENVELOPE_V1.md`
- `contracts/JOB_QUEUE_CONTRACT.md`
- `contracts/MODEL_GATEWAY_CONTRACT.md`
- `contracts/TOOL_CONTRACT.md`
- `contracts/SECRET_RESOLVER_CONTRACT.md`
- `contracts/TELEMETRY_CONTRACT.md`

## Platform
- `platform/AGENT_RUNTIME.md`
- `platform/EXECUTION_AND_WORKFLOWS.md`
- `platform/MODEL_GATEWAY.md`
- `platform/TOOLS_CONNECTORS_MCP.md`
- `platform/MEMORY_AND_KNOWLEDGE.md`
- `platform/OBSERVABILITY_AND_EVALUATION.md`
- `platform/POLICY_SECURITY_GOVERNANCE.md`
- `platform/AI_OFFICE_LAYER.md`
- `platform/API_SDK_EXTENSION_MODEL.md`

## Engineering
- `engineering/BUILD_VS_REUSE.md`
- `engineering/NON_FUNCTIONAL_REQUIREMENTS.md`
- `engineering/TESTING_STRATEGY.md`
- `engineering/DEVELOPMENT_GATES.md`
- `engineering/REPOSITORY_STRUCTURE.md`
- `engineering/SDK.md`
- `engineering/OSS_LICENSE_STRATEGY.md`

## Roadmap
- `roadmap/STAGE_ROADMAP.md`
- `roadmap/STAGE_COMPONENT_MATRIX.md`
- `roadmap/IMPLEMENTATION_TRACKER.md`
- `roadmap/OPEN_SOURCE_RELEASE_PLAN.md`

## Implementation
- `implementation/STAGE-0-FOUNDATION.md`
- `implementation/STAGE-1-COMMUNITY-ALPHA.md`
- `implementation/STAGE-2-COMMUNITY-BETA.md`
- `implementation/STAGE-3-OSS-1.0.md`

## ADRs
- `adr/ADR-001-target-architecture-first.md`
- `adr/ADR-002-ports-and-adapters.md`
- `adr/ADR-003-bullmq-stage1.md`
- `adr/ADR-004-durable-workflow-backend.md`
- `adr/ADR-005-apache2.md`
- `adr/ADR-006-trusted-typescript-runtime.md`
- `adr/ADR-007-runtime-protocol-and-remote-http.md`

## Source-of-truth precedence

1. accepted ADR;
2. architectural invariant;
3. versioned public contract;
4. subsystem architecture document;
5. roadmap/component matrix;
6. implementation-stage document.

No document in this repository contains private commercial packaging or internal company strategy.
