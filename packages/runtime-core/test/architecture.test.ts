...
const runtimeCoreSrc = path.join(repoRoot, "packages", "runtime-core", "src");
...
describe("architecture import restrictions", () => {
  it("keeps packages/runtime-core/src free of adapters, persistence, and orchestration", () => {
    const violations: string[] = [];

    for (const file of walkTypeScriptFiles(runtimeCoreSrc)) {
      for (const specifier of collectImportSpecifiers(
        fs.readFileSync(file, "utf8"),
      )) {
        const forbidden = forbiddenRuntimeCoreImport(specifier);
        if (forbidden) {
          violations.push(
