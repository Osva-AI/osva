...
function forbiddenDbImport(specifier: string): string | undefined {
  if (
  ...
  if (specifier === "prisma" || specifier.startsWith("prisma/")) {
    return "prisma";
  }

  if (specifier === "@prisma/client" || specifier.startsWith("@prisma/")) {
    return "prisma";
  }
  ...
  if (
    specifier === "testcontainers" ||
    specifier.startsWith("testcontainers/")
  ) {
...
describe("architecture import restrictions", () => {
  it("limits packages/db/src to domain, contracts, Drizzle, and postgres.js", () => {
    const violations: string[] = [];

    for (const file of walkTypeScriptFiles(dbSrc)) {
      for (const specifier of collectImportSpecifiers(
        fs.readFileSync(file, "utf8"),
      )) {
        const forbidden = forbiddenDbImport(specifier);
        if (forbidden) {
          violations.push(
  ...
  it("keeps domain and contracts free of @osva/db imports", () => {
    const violations: string[] = [];
